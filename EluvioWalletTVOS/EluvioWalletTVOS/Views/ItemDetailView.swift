                ...
                Task {
                  if let property = try await eluvio.fabric.getProperty(property: propertyId) {
                    ...
                    await MainActor.run {
                      ...
                      let params = PropertyParam(propertyId: property.id, pageId: page?.id)
                      router.path.append(.property(params))
                    }

                  } else {
                    debugPrint("Could not find property")
                    // router.path.append(.errorView("Could not find property."))
                  }
                }
              }) {
                Text("Go To Property")
...
#Preview("Item Description View") {
  ItemDescriptionView(
    name: "Sample NFT",
    subtitle: "Edition 1 #42",
    description: "This is a sample NFT description that shows the item details."
  )
  .padding()
  .background(Color.black)
}

#Preview("Mint Info View") {
  MintInfoView(mintInfo: [
    LabelValuePair(label: "Edition", info: "Standard"),
    LabelValuePair(label: "Number Minted", info: "500"),
    LabelValuePair(label: "Number in Circulation", info: "480"),
    LabelValuePair(label: "Cap", info: "1000"),
  ])
  .padding()
  .background(Color.black)
}

#Preview("Item Contract Info View") {
  ItemContractInfoView(
    address: "0x1234567890abcdef1234567890abcdef12345678",
    hash: "hq__abc123def456"
  )
