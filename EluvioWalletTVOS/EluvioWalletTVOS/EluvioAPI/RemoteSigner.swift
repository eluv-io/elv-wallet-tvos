...
class RemoteSigner {
  ...
  func getPropertyMultiview(propertyId: String, accessCode: String)
    async throws -> MultiviewResponse
  {
    return try await withCheckedThrowingContinuation { continuation in
      do {
        var endpoint = try self.getAuthEndpoint()
        ...
        guard let url = URL(string: endpoint) else {
          throw FabricError.invalidURL(
            "getPropertyMultiview - could not create url from \(endpoint)")
        }
        ...
        AF.request(request)
          ...
          .responseDecodable(of: MultiviewResponse.self) { response in
            ...
            case .failure(let error):
              var respJSON = JSON()
              do { respJSON = try JSON(data: response.data ?? Data()) } catch {}
              continuation.resume(
                throwing: FabricError.apiError(
                  code: response.response?.statusCode ?? 0,
                  response: respJSON, error: error))
  ...
  func refreshFabricToken(refreshToken: String, nonce: String, fabricToken: String) async throws
    -> JSON
  {
    return try await withCheckedThrowingContinuation { continuation in
