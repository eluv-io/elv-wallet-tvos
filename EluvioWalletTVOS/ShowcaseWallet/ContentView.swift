  ...
  func checkViewState() {
    ...
    Task {
      ...
      if viewState.op == .item {
        if let _nft = fabric.getNFT(
          contract: viewState.itemContract,
  ...
  var body: some View {
    if true {
      ...
      NavigationStack {
        ...
        .onDisappear {
          debugPrint("ContentView onDisappear")
          self.appeared = 0.0
          viewState.reset()
        }
        .onAppear {
      ...
      .fullScreenCover(isPresented: $showNft, onDismiss: didFullScreenCoverDismiss) {
        [backLink, backLinkIcon] in
      ...
      .fullScreenCover(isPresented: $showPlayer, onDismiss: didFullScreenCoverDismiss) {
        [backLink, backLinkIcon] in
        PlayerView(
          playerItem: self.$playerItem, seekTimeS: 0, finished: $playerFinished,
          backLink: backLink, backLinkIcon: backLinkIcon)
      }
      .fullScreenCover(isPresented: $showMinter, onDismiss: didFullScreenCoverDismiss) {
        [backLink, backLinkIcon] in
        MinterView(
          marketItem: $mintItem, mintInfo: $mintInfo,
          backLink: backLink, backLinkIcon: backLinkIcon)
      }
      .fullScreenCover(isPresented: $showProperty, onDismiss: didFullScreenCoverDismiss) {
        [backLink, backLinkIcon] in
