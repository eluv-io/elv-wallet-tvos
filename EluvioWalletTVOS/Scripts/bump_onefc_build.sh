#!/bin/bash
# Auto-increments the OneFC (com.eluvio.onefc) build number
...
# Only the OneFC target's Debug + Release configs are touched; the Mobile and
...
def bump_block(match):
    block = match.group(0)
    if "com.eluvio.onefc" not in block:
        return block
    return re.sub(
...
# matches one settings block at a time and only rewrites the OneFC ones.
open(path, "w").write(re.sub(r"\{[^{}]*\}", bump_block, text))
PY

echo "Bumped OneFC build number."
