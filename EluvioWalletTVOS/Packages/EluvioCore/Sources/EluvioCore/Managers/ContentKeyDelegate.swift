...
public class ContentKeyDelegate: NSObject, AVContentKeySessionDelegate {
  ...
  public func requestContentKeyFromKeySecurityModule(spcData: Data, assetID: String) throws -> Data {
    ...
    if let postData = postString.data(using: .ascii, allowLossyConversion: true),
      let drmServerUrl = URL(string: drmUrl)
    {
      ...
      URLSession.shared.dataTask(with: request) { data, _, error in
        if let data = data, var responseString = String(data: data, encoding: .utf8) {
          ...
          print(
            "Error encountered while fetching FairPlay license for URL: \(self.drmUrl), \(error?.localizedDescription ?? "Unknown error")"
          )
        }
  ...
  public func handleStreamingContentKeyRequest(keyRequest: AVContentKeyRequest) {
    ...
    let provideOnlinekey: () -> Void = { () in
        ...
        || persistableContentKeyExistsOnDisk(withContentKeyIdentifier: assetIDString)
      {
        ...
        do {
          print(
            "ContentKeyDelegate.handleStreamingContentKeyRequest respondByRequestingPersistableContentKeyRequestAndReturnError"
          )
