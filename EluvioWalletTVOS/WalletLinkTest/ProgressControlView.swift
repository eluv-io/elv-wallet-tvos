...
struct ProgressControlView: View {
  ...
  init(
    ...
    thumbs: [ThumbnailItem] = [], mediaItems: [InteractiveMediaItem] = [], page: Binding<Int>,
    numPages: Int, progress: Float, coverArtImage: UIImage?, next: (() -> Void)? = nil,
    previous: (() -> Void)? = nil, playAudio: Binding<Bool>, hasAudio: Bool
  ) {
    self.title = title
      ...
      VStack(alignment: .leading, spacing: 5) {
        ...
        Group {
          HStack(alignment: .center, spacing: 30) {
            ...
            if hasAudio {
              Image(
                systemName: playAudio
                  ? "speaker.wave.2.bubble.left.fill" : "speaker.wave.2.bubble.left"
              )
        ...
        if controlsLayout == .l2 {
          ...
          if selectedTab == .Info {
            InfoTab(
              image: coverArtImage ?? UIImage(), title: title, description: description,
              copyright: copyright
            )
          ...
          if selectedTab == .Thumbs {
            ThumbnailRowView(
              thumbs: thumbs, page: $page, thumbWidth: thumbWidth, thumbHeight: tabHeight * 0.8
            )
            .frame(maxWidth: .infinity, maxHeight: tabHeight)
          }
