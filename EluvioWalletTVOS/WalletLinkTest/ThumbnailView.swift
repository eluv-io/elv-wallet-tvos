    ...
    ScrollViewReader { value in
      ScrollView(.horizontal) {
        LazyHStack(spacing: thumbWidth * 0.2) {
          ThumbnailItemView(
            item1: thumbs[0], item2: nil, page: $page, width: thumbWidth, height: thumbHeight,
            selectedView: $selectedView)

          ForEach(Array(stride(from: 1, to: thumbs.count, by: 2)), id: \.self) { index in
            HStack(spacing: 0) {
              ThumbnailItemView(
                item1: thumbs[index], item2: index + 1 < thumbs.count ? thumbs[index + 1] : nil,
                page: $page, width: thumbWidth, height: thumbHeight, selectedView: $selectedView)
            }
            .id(index)
          }
        }
