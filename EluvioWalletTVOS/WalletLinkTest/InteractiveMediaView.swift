...
struct InteractiveMediaView: View {
  ...
  var gradient: LinearGradient {
    return LinearGradient(
      gradient: Gradient(colors: [
        .black.opacity(0.5), .black.opacity(0.9), .black.opacity(0.95), .black.opacity(1.0),
  ...
  var body: some View {
    ZStack {
      HStack(spacing: 0) {
        SyncedTextView(
          textItems: item.mainText, currentTimeMS: $currentTimeMS, selectedIndex: index)
        if item.type == .InteractiveVideo {
          PlayerView2(
            playoutUrl: item.mainLink,
            finished: $finished,
