...
struct PDFPage: View {
  ...
  init(
    title: String, url: URL? = nil, audioUrl: URL? = nil, description: String, copyright: String,
    coverArtUrl: URL?, mediaItems: [InteractiveMediaItem] = [], audioPages: [AudioPageModel] = [],
    enablePageTransitions: Bool = false
  ) {
    self.title = title
    self.url = url
  ...
  var body: some View {
      ...
      GeometryReader { _ in
        if isLoading {
          ...
          ZStack {
            ...
            ProgressControlView(
              title: title, description: description, copyright: copyright, thumbs: thumbs,
              mediaItems: mediaItems, page: $page, numPages: numPages, progress: progress,
    ...
    .edgesIgnoringSafeArea(.all)
    .onChange(of: page) { oldState, _ in
    ...
    .task {
      ...
      if let url = coverArtUrl {
        do {
          ...
          coverArtImage = imageForPDF(
            document: document, pageNumber: 1, imageHeight: documentHeight)
        }
      } else {
      ...
      for i in 0..<(self.document?.numberOfPages ?? 0) {
        if let image = imageForPDF(
          document: self.document!, pageNumber: i + 1, imageWidth: thumbWidth * 2)
        {
  ...
  func refreshPages() {
    ...
    Task {
      ...
      if page + 1 <= document.numberOfPages {
        if let _page2 = imageForPDF(
          document: document, pageNumber: page + 1, imageHeight: documentHeight)
        {
          page2 = _page2
        } else {
...
struct BookPageView: UIViewRepresentable {
  ...
  func makeUIView(context _: UIViewRepresentableContext<BookPageView>) -> UIImageView {
  ...
  func updateUIView(_ uiView: UIImageView, context _: UIViewRepresentableContext<BookPageView>) {
  ...
  func setImage(_ uiView: UIImageView, _ image: UIImage?, animated: Bool = true) {
    ...
    UIView.transition(
      with: uiView, duration: duration, options: .transitionCrossDissolve,
      animations: {
        uiView.image = image
      }, completion: nil)
  }
