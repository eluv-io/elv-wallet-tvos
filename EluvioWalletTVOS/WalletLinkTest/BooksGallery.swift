...
class BookItem: BooksDisplayItem {
  ...
  init(
    id: String? = nil, image: UIImage? = nil, posterImage: UIImage? = nil, name: String = "",
    description: String = "", copyright: String = "", pdfLink: URL? = nil,
    mainAudioLink: URL? = nil, audioPages: [AudioPageModel] = [],
    pageOverrides: [Int: PageOverride] = [:], animatePageTransitions: Bool = false
  ) {
...
class InteractiveAudioItem: InteractiveMediaItem {
  override init(
    id: String? = nil, image: UIImage? = nil, posterImage: UIImage? = nil, name: String = "",
    description: String = "", mainLink: URL? = nil, mainText: [TextItem] = []
  ) {
    super.init(
      id: id, image: image, posterImage: posterImage, name: name, description: description,
      mainLink: mainLink, mainText: mainText)
...
struct TVPoster: UIViewRepresentable {
  ...
  func makeUIView(context _: Context) -> TVPosterView {
  ...
  func updateUIView(_ uiView: TVPosterView, context _: Context) {
...
struct BooksGallery: View {
  ...
  var body: some View {
        ...
        ScrollView(.horizontal) {
          LazyHStack(spacing: 40) {
            ...
            ForEach(0..<books.count, id: \.self) { index in
              BooksGalleryItemView(
                item: $books[index], selected: $showBook, selectedItem: $selectedItem,
                width: imageWidth
              )
              .padding()
            }
    ...
    .onAppear {
      Task {
        ...
        do {
          guard let pdfUrl = Bundle.main.url(forResource: "AQP - opening", withExtension: "pdf")
          else {
            throw ("Could not get a-quiet-place-2018 url")
          }

          guard let audioUrl = Bundle.main.url(forResource: "AQP-audio", withExtension: "wav")
          else {
          ...
          guard
            let scriptCoverUrl = Bundle.main.url(forResource: "AQP-Script", withExtension: "png")
          else {
          ...
          guard
            let videoUrl = Bundle.main.url(
              forResource: "A Quiet Place - Opening", withExtension: "mp4")
          else {
        ...
        do {
          ...
          guard let page1Video = Bundle.main.url(forResource: "flash_page_1", withExtension: "mp4")
          else {
            debugPrint("Could not get flash_page_1 url")
            return
          }

          guard let page5Video = Bundle.main.url(forResource: "flash_page_5", withExtension: "mp4")
          else {
            debugPrint("Could not get flash_page_1 url")
            return
          }

          let pageOverrides: [Int: PageOverride] = [
            1: PageOverride(url: page1Video, page: 1), 5: PageOverride(url: page5Video, page: 5),
          ]
        ...
        do {
          guard
            let url = Bundle.main.url(
              forResource: "Entertainment Weekly - 05 July 2015", withExtension: "pdf")
          else {
  ...
  func createBookItem(
    name: String, description: String, posterImage: String, copyright: String, pdfLink: URL,
    pageOverrides: [Int: PageOverride] = [:], mainAudioLink: URL? = nil,
    audioPages: [AudioPageModel] = []
  ) throws -> BookItem {
    ...
    return BookItem(
      posterImage: img, name: name, description: description, copyright: copyright,
      pdfLink: pdfLink, mainAudioLink: mainAudioLink, audioPages: audioPages,
      pageOverrides: pageOverrides)
  }

  func createInteractiveItem(
    ...
    if type == .InteractiveVideo {
      return InteractiveVideoItem(
        image: img, posterImage: poster, name: name, description: description, mainLink: mainLink,
        mainText: textItems)
    } else if type == .InteractiveAudio {
      return InteractiveAudioItem(
        image: img, posterImage: poster, name: name, description: description, mainLink: mainLink,
        mainText: textItems)
    }
