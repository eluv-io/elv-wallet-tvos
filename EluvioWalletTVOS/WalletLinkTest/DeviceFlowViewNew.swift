...
struct DeviceFlowViewNew: View {
    ...
    VStack(alignment: .center, spacing: 30) {
      VStack(alignment: .center, spacing: 20) {
        Text("Scan QR Code")
          ...
          .fontWeight(.semibold)
        Text(
          "Scan the QR Code with your camera app or a QR code reader on your device to verify the code."
        )
  ...
  func regenerateCode() async {
    do {
      guard let signer = fabric.signer else {
      ...
      var url =
        fabric.getWalletBaseUrl()
        + "/login?mid=\(marketplaceId)&useOry=true&action=login&mode=login&response=code&source=code"
  ...
  func checkDeviceVerification(statusUrl _: String) async {
    ...
    do {
      guard let result = try await fabric.signer?.checkAuthLogin(createResponse: response) else {
