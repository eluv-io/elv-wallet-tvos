...
struct ProgressSlider: UIViewRepresentable {
  ...
  func makeUIView(context: UIViewRepresentableContext<ProgressSlider>) -> MyProgressView {
    ...
    let select = UITapGestureRecognizer(
      target: context.coordinator, action: #selector(Coordinator.select))
    select.delegate = context.coordinator
    select.allowedPressTypes = [NSNumber(value: UIPress.PressType.select.rawValue)]

    let left = UITapGestureRecognizer(
      target: context.coordinator, action: #selector(Coordinator.left))
    left.delegate = context.coordinator
    left.allowedPressTypes = [NSNumber(value: UIPress.PressType.leftArrow.rawValue)]

    let right = UITapGestureRecognizer(
      target: context.coordinator, action: #selector(Coordinator.right))
    ...
    let down = UITapGestureRecognizer(
      target: context.coordinator, action: #selector(Coordinator.down))
    ...
    let upSwipeRecognizer = UISwipeGestureRecognizer(
      target: context.coordinator, action: #selector(Coordinator.up))
  ...
  func updateUIView(_ uiView: MyProgressView, context _: UIViewRepresentableContext<ProgressSlider>)
  {
  ...
  class Coordinator: NSObject, UIGestureRecognizerDelegate {
    ...
    @objc func left(gesture _: UITapGestureRecognizer) {
      print("left arrow")
    ...
    @objc func right(gesture _: UITapGestureRecognizer) {
      print("right arrow")
      // parent.page = parent.page + 2
    ...
    @objc func up(gesture _: UITapGestureRecognizer) {
      print("up arrow")
      // parent.page = parent.page + 2
    ...
    @objc func down(gesture _: UITapGestureRecognizer) {
      print("down arrow")
      // parent.page = parent.page + 2
    ...
    @objc func select(gesture _: UITapGestureRecognizer) {
      print("select")
      // parent.page = parent.page + 2
